    ...
    for payload in sql_injection_payloads:
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, list):
                assert len(data) <= 10, \
                    f"Posible SQL Injection: se devolvieron {len(data)} facturas con payload: {payload}"
...
def test_sql_injection_comment_based_attack(auth_token):
    ...
    for payload in comment_payloads:
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, list):
                assert len(data) <= 20, \
                    f"Posible bypass de condiciones: se devolvieron {len(data)} facturas"
...
def test_sql_injection_special_characters_and_encoding(auth_token):
    ...
    for payload in special_char_payloads:
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, list):
                assert len(data) <= 15, \
                    f"Respuesta sospechosa con {len(data)} facturas para payload: {payload}"
